...
static void Array_ctor(ArrayClass *this, va_list *args)
{
    ...
    if (strcmp(class->__name__, "Float") != 0)
        arg = va_arg(*args, void *);
    else
        float_arg = (float)va_arg(*args, double);
    this->_type = class;
    this->_size = size;
    this->_tab = malloc(sizeof(*class) * (size + 1));
    if (!this->_tab)
        raise("Array construction failed (memory)\n");
    this->_tab[size] = NULL;
    for (size_t i = 0; i < size; i++) {
        if (strcmp(class->__name__, "Float") != 0)
            this->_tab[i] = new(class, arg);
        else
            this->_tab[i] = new(class, float_arg);
    }
}
...
static Object *Array_getitem(ArrayClass *this, ...)
{
    ...
    va_start(my_list, this);
    pos = va_arg(my_list, size_t);
    va_end(my_list);
    if (pos > this->_size)
        return NULL;
    for (; i < pos && this->_tab[i] != NULL; i++);
    if (this->_tab[i] == NULL)
        return NULL;
    return this->_tab[i];
}
...
static void Array_setitem(ArrayClass *this, ...)
{
    ...
    if (this->_tab[i] == NULL)
        return;
    delete(this->_tab[i]);
    this->_tab[i] = va_new(this->_type, &my_list);
    va_end(my_list);
}
